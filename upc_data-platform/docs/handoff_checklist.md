...
MINIO_ROOT_USER, MINIO_ROOT_PASSWORD, MINIO_PORTAL_USER, MINIO_PORTAL_PASSWORD, MINIO_AIRFLOW_USER, MINIO_AIRFLOW_PASSWORD

[ ] Confirm Airflow variables exist:
AIRFLOW_DB_USER, AIRFLOW_DB_PASSWORD, AIRFLOW_WEB_USER, AIRFLOW_WEB_PASSWORD

[ ] Confirm Superset variables exist:
SUPERSET_DB_USER, SUPERSET_DB_PASSWORD, SUPERSET_WEB_USER, SUPERSET_WEB_PASSWORD, SUPERSET_SECRET_KEY
...
```
  ...
  **Expected services:** `postgres`, `minio`, `minio-init`, `airflow-webserver`, `airflow-scheduler`, `data-portal`, `superset`
  > **Note:** Some init containers may exit after completing their setup. This is normal.
...
```
[ ] Expected result: process_new_data_dag, No data found, paused = False

[ ] Open Airflow: http://localhost:8080

[ ] Login using: AIRFLOW_WEB_USER / AIRFLOW_WEB_PASSWORD
...
[ ] Open Streamlit data portal: http://localhost:8501

[ ] Login using: PORTAL_ACCESS_PASSWORD
...
```
  ...
  **Expected raw tables:** `public.raw_cases`, `public.raw_patents`, `public.raw_parties`
...
[ ] Login using: SUPERSET_WEB_USER / SUPERSET_WEB_PASSWORD

[ ] Add PostgreSQL database connection if needed:

Plaintext
postgresql+psycopg2://project_user:project_pass@postgres:5432/project_db
[ ] Confirm Superset can access: raw_cases, raw_patents, raw_parties

[ ] Use SQL files from superset/sql/ to recreate analytical datasets if needed.
...
## 9. Utility Scripts
[ ] Airflow check works: .\scripts\check_airflow.ps1

[ ] PostgreSQL check works: .\scripts\check_postgres.ps1

[ ] Full smoke test works: .\scripts\smoke_test.ps1

[ ] Safe restart works: .\scripts\reset_project.ps1
...
## 13. Deployment / CD Check
[ ] The repository is cloned on the deployment machine.

[ ] .env is created from .env.example.

[ ] .\scripts\deploy.ps1 runs successfully.

[ ] Docker Compose starts all services.

[ ] Airflow DAG is visible and unpaused.

[ ] Superset health check passes.

[ ] Streamlit portal is reachable.

[ ] A test Excel upload triggers the Airflow DAG.